...
static int init_types(astmodulestate *state)
{
    ...
    state->stmt_type = make_type(state, "stmt", state->AST_type, NULL, 0,
        ...
        "     | Sandbox(constant uid, string mem, string sys, stmt* body)\n"
...
ast2obj_stmt(astmodulestate *state, void* _o)
{
    ...
    case Sandbox_kind:
        ...
        result = PyType_GenericNew(tp, NULL, NULL);
        if (!result) goto failed;
        value = ast2obj_constant(state, o->v.Sandbox.uid);
        if (!value) goto failed;
        if (PyObject_SetAttr(result, state->uid, value) == -1)
            goto failed;
        Py_DECREF(value);
...
obj2ast_stmt(astmodulestate *state, PyObject* obj, stmt_ty* out, PyArena* arena)
{
    ...
    if (isinstance) {
        ...
        if (_PyObject_LookupAttr(obj, state->uid, &tmp) < 0) {
            return 1;
        }
        if (tmp == NULL) {
            ...
            int res;
            res = obj2ast_constant(state, tmp, &uid, arena);
            if (res != 0) goto failed;
            Py_CLEAR(tmp);
        }
